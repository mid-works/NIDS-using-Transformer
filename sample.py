...
class NIDSPreprocessor:
    ...
    def _preprocess_features(self, df, is_training=True):
        ...
        numeric_cols_for_scaling = [
            ...
            if c != 'label' and c in self.feature_names
        ]
...
class RealTimeFlowMonitor:
    def __init__(self, flow_timeout=CONFIG["FLOW_TIMEOUT_SECONDS"],
        ...
        self.flow_expiry_queue = deque() # Simple counter for flow identification
    ...
    def _generate_features_loop(self):
        while self.is_monitoring:
            ...
            with self.lock:
                while self.flow_expiry_queue:
                    flow_key = self.flow_expiry_queue[0]
                    flow_data = self.active_flows.get(flow_key)

                    if not flow_data:
                        self.flow_expiry_queue.popleft()
                        continue
                        
                    ts = flow_data['last_time']
                    if now - ts < self.flow_timeout:
                        break
                    ...
                    if flow_data['src_pkts'] > 0 or flow_data['dst_pkts'] > 0:
                            try:
                                ...
                                self.processed_flow_features.append(features)
                                self.total_flows_processed += 1
                            except Exception as e:
                                logging.exception(f"Flow feature calc error: {e}")  
            time.sleep(0.5)
...
class NIDSMonitorGUI:
    ...
    def _update_graphs(self, current_stats):
        ...
        try:
            ...
            x_data = list(range(len(rate_data)))

            self.rate_line.set_data(x_data, rate_data)

            self.ax1.relim()
            self.ax1.autoscale_view(scalex=True, scaley=True)
            self.canvas1.draw_idle()

        # ---------- Flow classification bar graph ----------
            normal_flows = max(0, self.total_flows_analyzed - self.attack_count)
            attack_flows = self.attack_count

            self.classification_bars[0].set_height(normal_flows)
            self.classification_bars[1].set_height(attack_flows)

            self.ax2.relim()
            self.ax2.autoscale_view(scalex=False, scaley=True)

        # Keep Y-axis sensible
            max_val = max(normal_flows, attack_flows, 1)
...
def main():
    """Initializes components and starts the NIDS application (DEPLOYMENT SAFE)."""
    PERFORM_RETRAINING = False  # MUST be False for deployment
    ...
    try:
        ...
        # ===============================
        # STEP 1: Try loading trained model
        # ===============================
        model_loaded = detector.load_model()

        if model_loaded and not PERFORM_RETRAINING:
            logging.info("Running in DEPLOYMENT MODE (no dataset required).")

            # Load scaler
            if not os.path.exists(CONFIG["SCALER_PATH"]):
                logging.error("Scaler file missing. Dataset required.")
                return
            preprocessor.scaler = joblib.load(CONFIG["SCALER_PATH"])

            # Load selected features
            if not os.path.exists(CONFIG["FEATURE_PATH"]):
                logging.error("Selected features file missing. Dataset required.")
                return
            feature_engineer.selected_features = joblib.load(CONFIG["FEATURE_PATH"])

            # Set feature names for preprocessing
            preprocessor.feature_names = feature_engineer.selected_features

        else:
            # ===============================
            # TRAINING MODE (dataset required)
            # ===============================
            logging.warning("Entering TRAINING MODE (dataset required).")
            ...
            try:
                ...
                feature_engineer.select_features(X_train, y_train)
                detector.train(X_train, y_train)

            except FileNotFoundError:
                logging.error(f"Dataset not found at {CONFIG['DATA_PATH']}")
                return
            except Exception as e:
        ...
        if not preprocessor.feature_names:
            logging.error("Preprocessor not initialized properly.")
            return

        if not feature_engineer.selected_features:
            logging.error("FeatureEngineer not initialized properly.")
            return

        logging.info("All components ready. Launching GUI...")
